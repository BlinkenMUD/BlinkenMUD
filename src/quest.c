...
QUEST_BLIST quest_item_list[] = {	/* List of items that you can buy with quest points.
  ...
  ,
  {"30 Practices...............", "pract prac practices", 500, 30,
   QUEST_PRAC_T}
  ,
  {"5 Trainings................", "train training trainings", 500, 5,
   QUEST_TRAIN_T}
  ,
  /* END OF LIST */
  {NULL, NULL, -1, -1, -1}
...
do_quest (CHAR_DATA * ch, char *argument)
{
  ...
  if (!strcmp (arg1, "info"))
      ...
      if (IS_SET (ch->act, PLR_QUESTOR))
	{
	  if (ch->questmob == -1 && ch->questgiver->short_descr != NULL)
	    ...
	    {
	      sprintf (buf,
		       "Well this ain't supposed to happen, you'll be reset to normal state but please report to admin!\n\r");
	      send_to_char (buf, ch);
	      ch->nextquest = 0;
  ...
  if (!strcmp (arg1, "list"))
	...
	{
	  sprintf (buf, "%s.......%d qp\n", quest_item_list[i].name,
		   quest_item_list[i].cost);
	  send_to_char (buf, ch);
	  i++;
	}
      return;
	  ...
	  if (is_name (arg2, quest_item_list[i].mname))
	      ...
	      if (ch->questpoints > quest_item_list[i].cost)
		{
		  ...
		  switch (quest_item_list[i].type)
		    ...
		    case QUEST_OBJ_T:
		      obj =
			create_object (get_obj_index (quest_item_list[i].id),
				       ch->level);
		    ...
		    case QUEST_GOLD_T:
		      ...
		      sprintf (buf, "$N gives %d pieces of gold to $n.",
			       quest_item_list[i].id);
		      act (buf, ch, NULL, questman, TO_ROOM);
		      sprintf (buf,
			       "$N has %d in gold transfered from $s Swiss account to your balance.",
			       quest_item_list[i].id);
		      act (buf, ch, NULL, questman, TO_CHAR);
		      break;
		    case QUEST_PRAC_T:
		      ch->practice += quest_item_list[i].id;
		      sprintf (buf, "$N gives %d practices to $n.",
			       quest_item_list[i].id);
		      act (buf, ch, NULL, questman, TO_ROOM);
		      sprintf (buf, "$N gives you %d practices.",
			       quest_item_list[i].id);
		    ...
		    case QUEST_TRAIN_T:
		      ...
		      sprintf (buf, "$N gives %d trainings to $n.",
			       quest_item_list[i].id);
		      act (buf, ch, NULL, questman, TO_ROOM);
		      sprintf (buf, "$N gives you %d trainings.",
			       quest_item_list[i].id);
		      act (buf, ch, NULL, questman, TO_CHAR);
		      break;
		    }
...
generate_quest (CHAR_DATA * ch, CHAR_DATA * questman)
{
  ...
  /* Quest to kill a mob */
	...
	case 1:
	  sprintf (buf,
		   "Rune's most heinous criminal, %s, has escaped from the dungeon!",
		   victim->short_descr);
	  do_say (questman, buf);
