...
/* IMM's, pet's, and npc's dont need bank accounts so we will remove them */
   ...
   {
   sprintf( buf, "You have Platinum: %10ld Gold: %10ld Silver: %10ld in your account.\n\r",plat, gold, silver);
   send_to_char(buf, ch);
   }
 }
        ...
        if(!str_cmp( arg2, "plat"))
         {
           if (ch->platinum < amount)
            ...
            else
            {
              ...
              ch->platinum -= amount;
              act("$n has deposited some platinum into $s account.", ch, NULL, NULL, TO_ROOM);
              sprintf( buf, "You have deposited %ld Platinum.\n\r   Account: %10ld.\n\r   You still hold %8ld platinum.\n\r", 
...
ch->pcdata->silver_bank, ch->silver);
	      send_to_char( buf, ch);
	      return;
	    }
	  }
             ...
             if(!str_cmp( arg2, "plat"))
                {
                  if (ch->pcdata->plat_bank < amount)
                     {
                       send_to_char("You dont have that much platinum in your account.\n\r", ch);
                       return;
                     }
                    else
                     {
