 ...
 ****************************************************************************/
void
show_skill_cmds (CHAR_DATA * ch, int tar)
{
  char buf[MAX_STRING_LENGTH];
 ...
 */
void
show_obj_values (CHAR_DATA * ch, OBJ_INDEX_DATA * obj)
{
  char buf[MAX_STRING_LENGTH];
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
    ...
    case ITEM_PIT:
      sprintf (buf,
	       ...
	       "[v4] Weight Mult [%d]\n\r",
	       obj->value[0],
	       flag_string (pit_flags, obj->value[1]),
	       get_obj_index (obj->value[2])
	       ? get_obj_index (obj->value[2])->short_descr
	       : "none", obj->value[2], obj->value[3], obj->value[4]);
      send_to_char (buf, ch);
...
/* WEAPONS changed in ROM */
	...
	case 0:
	  ...
	  break;
	case 1:
	  if ((value = flag_value (pit_flags, argument)) != NO_FLAG)
	    TOGGLE_BIT (pObj->value[1], value);
	  else
	    {
	      do_help (ch, "ITEM_PIT");
 ...
 ****************************************************************************/
bool
oedit_values (CHAR_DATA * ch, char *argument, int value)
{
  OBJ_INDEX_DATA *pObj;
