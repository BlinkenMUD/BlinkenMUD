...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    {
	 if( ch->pcdata->hunt_time < current_time )	reset_hunt(ch);
	 else	strcat( buf, "{D({rS{Rt{Wolen Au{Rr{ra{D){x " );
    }
    else

    if ( IS_OBJ_STAT(obj, ITEM_SHARP) )   strcat( buf, "{C({DS{dh{w{War{Dp{C){x" );
	...
	if ( IS_OBJ_STAT(obj, ITEM_SHARP) 	)   buf[26] = 'S';
	if (!strcmp(buf, "{x[{y.{R.{B.{M.{Y.{W.{G.{C.{x]") ) buf[0] = '\0';
    }
    else
    {
