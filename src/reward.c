...
CHAR_DATA *
find_captain (CHAR_DATA * ch)
{
  CHAR_DATA *captain;

  for (captain = ch->in_room->people; captain;
       captain = captain->next_in_room)
    {
...
/* adapted from healer.c */
CHAR_DATA *
find_rewarder (CHAR_DATA * ch)
{
  CHAR_DATA *rewarder;

  for (rewarder = ch->in_room->people; rewarder;
       rewarder = rewarder->next_in_room)
    {
      if (IS_NPC (rewarder)
	  && rewarder->pIndexData->vnum == ch->pcdata->rewarder)
	break;
    }
  if (rewarder == NULL)
    {
...
void
bounty (CHAR_DATA * ch, CHAR_DATA * captain)
{
  ...
  for (vnum = 1202; vnum < 30001; vnum = number_range (100, 30000))
      ...
      if ((room = get_room_index (vnum)) != NULL)
	{
	  ...
	  aggie_found = FALSE;
	  for (victim = room->people; victim != NULL;
	  ...
	  if (!aggie_found)
		   ...
		   victim = victim->next_in_room)
		{
		  if (IS_NPC (victim)
		      && (IS_NEUTRAL (victim)
			  || (ch->alignment >
			      0 ? IS_EVIL (victim) : IS_GOOD (victim)))
		      ...
		      && !IS_SET (victim->affected_by, AFF_CHARM))
		    {
		       ...
		       */
		      obj_found = FALSE;
		      for (obj = victim->carrying; obj != NULL;
			   obj = obj_next)
			{
  ...
  switch (obj->item_type)
    ...
    default:
      sprintf (buf,
	       ...
	       victim->short_descr, victim->short_descr, obj->short_descr);
      break;
    }
  do_say (captain, buf);
  ...
  if (is_exact_name (first, "the a an"))
    ...
    if (is_exact_name
	(first,
	 "on in at by near inside under within approaching standing climbing swimming sailing walking sliding floating close lost along going over near very atop beneath before below between"))
    {
      prep = "";
      where = decap (victim->in_room->name);
    }
  ...
  if (obj_found && (rewarder = find_rewarder (ch)) != NULL)
    {
      reward =
	((ch->level + 50) / 3) +
	((ch->pcdata->hunt_time - current_time) / 20) +
	(UMAX ((get_mob_index (ch->pcdata->bounty_mob)->level - ch->level), 0)
	 * 10);
	...
	case 3:
      ...
      act ("$c takes $p from $N and gives $M the bounty.", rewarder, obj, ch,
	   TO_NOTVICT);
      ...
      if (obj_found)
	{
	  ...
	  if (ch->pcdata->rewarder > 0)
	  ...
	  else
	    {
	      ...
	      /* start with a random number */
	      for (vnum = 1202; vnum < 30001;
		   vnum = number_range (100, 30000))
		{
		  /*make it a random room */
		  if ((room = get_room_index (vnum)) != NULL)
		    {
		      for (rewarder = room->people; rewarder != NULL;
			   rewarder = rewarder->next_in_room)
			{
			  if (IS_NPC (rewarder) && ch->level + (ch->level / 3) + 3 >= rewarder->level && !IS_SET (rewarder->act, ACT_AGGRESSIVE)	/* by popular demand! */
	  ...
	  if (is_exact_name
	      (arg,
	       "on in at by near inside under within approaching standing climbing flying sailing swimming falling close shipwrecked along going over near very beneath before way"))
	    art = "";
	  else
	    art = "at ";
	  ...
	  sprintf (buf,
		   ...
		   ch->name,
		   get_obj_index (ch->pcdata->bounty_obj)->short_descr,
		   get_mob_index (ch->pcdata->rewarder)->short_descr, art,
